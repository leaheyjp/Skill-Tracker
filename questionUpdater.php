...
<form action="Instructor_Profile.php">
<?php
...
$i = 0;
$sqlmax = "SELECT COUNT(DISTINCT Question_ID) FROM Questions NATURAL JOIN Skills WHERE Skill_Name = '" . $_POST['skill'] . "';";
$resultmax = mysqli_query($con,$sqlmax);
$max = mysqli_fetch_array($resultmax)[0];


while($i < $max)
{
    $sql = "UPDATE Questions 
          SET Text = '" . $_POST['question' . (String)$i ] .  "'
          WHERE Questions.Skill_Name = '" . $_POST['skill'] . "' AND Question_ID='" . $_POST['ID' . $i] . "';";
    echo $sql;
